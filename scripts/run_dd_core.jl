...
jobs = dd_jobs(run_seed; config)
results = pmap(run_dd_job, jobs; batch_size=1)
...
frame.evaluation_design = fill("independent_holdout", nrow(frame))
CSV.write(destination, frame)

println("Wrote $(nrow(frame)) DD parameter results to $destination")
