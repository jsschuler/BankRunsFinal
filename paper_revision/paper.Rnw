...
analysis_root <- Sys.getenv(
  "BANKRUNS_ANALYSIS_ROOT",
  unset = "/Users/l25-n05917-res/ResearchCode/BankRunsFinal"
)
run_root <- file.path(analysis_root, "runs", "network_20260723")

dd_run_root <- file.path(
  analysis_root, "runs", "dd_overnight_20260724"
)
dd_fixed <- fread(file.path(
  dd_run_root, "fixed_sensitivity", "results.csv"
))
dd_allocation <- fread(file.path(
  dd_run_root, "allocation_main", "results.csv"
))
dd_shift_allocation <- fread(file.path(
  dd_run_root, "allocation_shift", "results.csv"
))
dd_bridge <- fread(file.path(
  analysis_root, "output", "dd_bridge_bounds_seed20260723.csv"
))
network_validation <- fread(file.path(
  analysis_root, "output", "network_validation_seed20260723.csv"
))
network_confirmatory <- fread(file.path(
  run_root, "confirmatory", "results.csv"
))
network_monotonicity <- fread(file.path(
  run_root, "monotonicity", "results.csv"
))
monotonicity_design <- fread(file.path(
  run_root, "monotonicity", "design.csv"
))

stopifnot(
  nrow(dd_fixed) == 41600L,
  nrow(dd_allocation) == 6400L,
  nrow(dd_shift_allocation) == 1440L,
  nrow(dd_bridge) == 360L,
  nrow(network_validation) == 1800L,
  nrow(network_confirmatory) == 96000L,
  nrow(network_monotonicity) == 39600L,
  uniqueN(network_confirmatory$job_id) == 96000L,
  uniqueN(network_monotonicity$job_id) == 39600L
)
...
paired_rules <- function(d, id_columns, model_column) {
  wide <- dcast(
    d,
  ...
  data.table(
    threshold = mean(wide$threshold),
    explicit = mean(wide$explicit_utility),
    difference = mean(wide$threshold) - mean(wide$explicit_utility),
    agreement = mean(wide$threshold == wide$explicit_utility)
  )
}

rules_validation <- paired_rules(network_validation, "job_index", "model")
rules_confirmatory <- paired_rules(
  network_confirmatory, c("structural_index", "replication"), "decision_model"
)
rules_monotonicity <- paired_rules(
  network_monotonicity, c("structural_index", "replication"), "decision_model"
)

dd_fixed_baseline <- dd_fixed[utility_shift == 1]
dd_fixed_premium_summary <- dd_fixed_baseline[
  , .(
...
dd_allocation_premium_summary <- dd_allocation[
  , .(
...
dd_high_failure <- dd_allocation[
  bank_formed & failure_rate >= 0.10
]
dd_shift_common <- rbindlist(list(
  dd_allocation[
    rho %in% c(.5, 1, 2) &
      withdrawal_premium %in% c(.01, .05, .10, .50) &
      productivity %in% c(.50, 1) &
      abs(withdrawal_probability * 10 -
        round(withdrawal_probability * 10)) < 1e-8,
    .(
      rho, withdrawal_premium, productivity, withdrawal_probability,
      utility_shift, optimal_deposit, bank_formed
    )
  ],
  dd_shift_allocation[
    abs(withdrawal_probability * 10 -
...
dd_shift_cell <- dd_shift_common[
  , .(
    deposit_agreement = uniqueN(optimal_deposit) == 1L,
    participation_agreement = uniqueN(bank_formed) == 1L,
    deposit_range = max(optimal_deposit) - min(optimal_deposit)
  ),
  by = .(rho, withdrawal_premium, productivity, withdrawal_probability)
]
network_prespecified_summary <- network_monotonicity[
  , .(
...
bridge_wide <- dcast(
  dd_bridge, scenario_index ~ bound, value.var = "withdraw"
)
bridge_agree <- bridge_wide[
  staying_favoring == withdrawal_favoring
]
bridge_withdraw <- bridge_agree[staying_favoring == TRUE, .N]
bridge_stay <- bridge_agree[staying_favoring == FALSE, .N]
bridge_sensitive <- bridge_wide[
  staying_favoring != withdrawal_favoring, .N
]
...
and explicit-utility rules agree on \Sexpr{fmt_pct(rules_validation$agreement)}
of common-scenario validation outcomes and
\Sexpr{fmt_pct(rules_monotonicity$agreement)} of a prespecified
...
\Sexpr{fmt_pct(dd_fixed_premium_summary[
  withdrawal_premium == .005, failure_rate])}
at a premium of 0.005 to
\Sexpr{fmt_pct(dd_fixed_premium_summary[
  withdrawal_premium == .50, failure_rate])}
...
\begin{figure}[ht]
\centering
<<dd-core-figure, fig.width=7.5, fig.height=4.8, out.width='0.88\\textwidth'>>=
dd_plot <- dd_fixed_baseline[
  withdrawal_premium %in% c(0, .005, .02, .10, .25, .50),
  .(failure_rate = mean(failure_rate)),
...
ggplot(
  ...
  aes(
    ...
    linetype = factor(withdrawal_premium)
  )
  ) +
  geom_line(linewidth = 0.7) +
  facet_wrap(~rho, labeller = label_both, ncol = 2) +
  scale_color_brewer(palette = "Spectral", name = "Withdrawal premium") +
  scale_linetype_discrete(name = "Withdrawal premium") +
  labs(
...
\Sexpr{fmt_pct(dd_allocation_premium_summary[
  withdrawal_premium == 0, participation_rate])}
at zero premium to
\Sexpr{fmt_pct(dd_allocation_premium_summary[
  withdrawal_premium == .50, participation_rate])}
at a premium of 0.50, while the mean selected deposit falls from
\Sexpr{sprintf("%.1f", dd_allocation_premium_summary[
  withdrawal_premium == 0, mean_deposit])} to
\Sexpr{sprintf("%.1f", dd_allocation_premium_summary[
  withdrawal_premium == .50, mean_deposit])}.
...
\begin{figure}[ht]
\centering
<<dd-allocation-figure, fig.width=7.5, fig.height=5.2, out.width='0.88\\textwidth'>>=
dd_allocation_plot <- dd_allocation[
  , .(
    Participation = mean(bank_formed),
    `Mean deposit / 1000` = mean(optimal_deposit) / 1000
  ),
  by = .(withdrawal_premium, rho)
]
dd_allocation_plot <- melt(
  dd_allocation_plot,
  id.vars = c("withdrawal_premium", "rho"),
...
ggplot(
  ...
  scale_color_brewer(palette = "Dark2", name = expression(rho)) +
  scale_x_continuous(breaks = c(0, .1, .25, .5)) +
  labs(
    x = "Withdrawal premium",
    y = NULL
  ) +
  theme_bw(base_size = 10) +
  theme(legend.position = "bottom")
...
fragile cells. Among the \Sexpr{nrow(dd_high_failure)} participating
cells with failure rates of at least 10 percent, selected deposits
range from \Sexpr{min(dd_high_failure$optimal_deposit)} to
\Sexpr{max(dd_high_failure$optimal_deposit)}. Under nonlinear utility,
...
\Sexpr{fmt_pct(min(dd_shift_cell[
  , mean(deposit_agreement), by = .(rho, withdrawal_premium)]$V1))}
in every risk-aversion/premium group, and the largest within-cell
deposit range is \Sexpr{max(dd_shift_cell$deposit_range)} units out
...
\begin{figure}[ht]
\centering
<<bridge-cutoff-figure, fig.width=7.2, fig.height=4.4, out.width='0.84\\textwidth'>>=
pstar_value <- function(rho, return_value) {
  ifelse(
    abs(rho - 1) < 1e-10,
    log(2) / log(1 + return_value),
    (2^(1 - rho) - 1) /
      ((1 + return_value)^(1 - rho) - 1)
  )
}
pstar_grid <- CJ(
  rho = c(0, 0.5, 1, 1.5, 2),
  return_value = seq(1.05, 2.5, length.out = 150)
)
pstar_grid[, cutoff := pstar_value(rho, return_value)]
ggplot(
  pstar_grid,
  aes(return_value, cutoff, color = factor(rho), linetype = factor(rho))
) +
  geom_line(linewidth = 0.8) +
  labs(
    x = "Gross long-asset return, R",
    y = "Recovery-probability cutoff, p*",
...
partial-payment bounds agree in \Sexpr{nrow(bridge_agree)} of 180
...
The two bounds agree in \Sexpr{nrow(bridge_agree)} of 180 scenarios.
Both prescribe withdrawal in \Sexpr{bridge_withdraw} scenarios and
staying in \Sexpr{bridge_stay}; only \Sexpr{bridge_sensitive}
...
\begin{figure}[ht]
\centering
<<bridge-classification-figure, fig.width=7.6, fig.height=5.2, out.width='0.9\\textwidth'>>=
bridge_plot_data <- unique(dd_bridge[, .(
  scenario_index,
  withdrawal_probability,
  withdrawal_premium,
  productivity,
...
bridge_plot_data <- merge(
  bridge_plot_data,
  bridge_wide,
  by = "scenario_index",
  all.x = TRUE
)
bridge_plot_data[, classification := fifelse(
  staying_favoring == withdrawal_favoring,
  fifelse(staying_favoring, "Withdraw under both bounds",
          "Stay under both bounds"),
  "Sensitive to partial-payment bound"
)]
ggplot(
  bridge_plot_data,
  aes(withdrawal_probability, bridge_threshold, color = classification)
) +
  geom_point(size = 2.1, alpha = 0.85) +
  facet_grid(withdrawal_premium ~ productivity, labeller = label_both) +
  scale_color_manual(
    values = c(
      "Withdraw under both bounds" = "#D55E00",
      "Stay under both bounds" = "#0072B2",
      "Sensitive to partial-payment bound" = "#777777"
    ),
    name = NULL
  ) +
  labs(
    x = "Prior withdrawal probability",
    y = "Recovery-probability cutoff"
  ) +
...
<<bridge-table, results='asis'>>=
bridge_table <- data.table(
  Classification = c(
    "Both bounds: withdraw",
    "Both bounds: stay",
    "Bound-sensitive"
  ),
  Scenarios = c(bridge_withdraw, bridge_stay, bridge_sensitive)
)
kable(
  bridge_table,
  format = "latex",
...
\end{equation}
%
Agent $i$ treats $\hat{w}_i(t)$ as a point estimate of the withdrawal
probability among its still-banking peers, floored at the baseline
rate $p_0$ so that an uninformative or empty neighborhood defaults to
the baseline rather than zero:
%
\begin{equation}
...
\end{equation}
%
Agent $i$ forms a belief about how many of the $n_i(t)$ currently
still-banking peers other than itself will withdraw before the
process settles:
%
\begin{equation}
...
\end{equation}
%
This is a plug-in point estimate, not a posterior: it carries no
representation of estimation uncertainty, and the floor at $p_0$
means a single withdrawn neighbor can move $\hat{p}_i(t)$
discontinuously rather than shrinking it gradually toward $p_0$ as
the local sample grows.

\paragraph{Belief-model qualification.} $\hat{p}_i(t)$ is a
deliberately simple heuristic, chosen for computational directness
across hundreds of thousands of decision draws in a 200-agent
simulation, not as a claim that it is the unique rational or
empirically correct belief process. Other belief laws for the same
local information are possible, including ones with genuine posterior
updating; this modeling choice is not asserted to be uniquely
determined by the theory in Sections~\ref{sec:dd}
and~\ref{sec:bridge}, which fix only the recovery-probability
threshold compared against, not how an agent estimates the
probabilities entering that comparison.
...
Exogenous withdrawals occur at the start of each simulation run. The
number of exogenously withdrawing agents, the shock size, is a fixed
experimental design parameter rather than a random draw; it is one of
the swept comparative-statics dimensions reported in
Section~\ref{sec:results} (for example, one-agent versus ten-agent
shocks). Given the shock size, withdrawing agents are selected either
uniformly at random across the network or via a localized rule that
seeds the shock at one randomly chosen agent and expands outward
through that agent's network neighbors until the target count is
reached; both placements are compared in Section~\ref{sec:results}.
...
<<network-rule-table, results='asis'>>=
network_table <- copy(network_prespecified_summary)
network_table[, decision_model := c(
  comparative = "Comparative recovery",
  explicit_utility = "Explicit utility",
  threshold = "Bridge threshold"
)[decision_model]]
network_table[, failure_rate := sprintf("%.3f", failure_rate)]
network_table[, contagion_probability := sprintf("%.3f", contagion_probability)]
network_table[, mean_endogenous_withdrawals :=
  sprintf("%.1f", mean_endogenous_withdrawals)]
setnames(
  network_table,
  c(
    "decision_model",
...
\Sexpr{fmt_pct(rules_validation$agreement)} of cases. Their failure
rates are \Sexpr{fmt_pct(rules_validation$threshold)} and
\Sexpr{fmt_pct(rules_validation$explicit)}, a difference of only
\Sexpr{fmt_pct(abs(rules_validation$difference))}. In the broader
prespecified monotonicity sweep, outcome agreement remains
\Sexpr{fmt_pct(rules_monotonicity$agreement)} and the aggregate
failure-rate difference is
\Sexpr{fmt_pct(abs(rules_monotonicity$difference))}.
...
\begin{figure}[ht]
\centering
<<network-validation-figure, fig.width=7.5, fig.height=4.6, out.width='0.88\\textwidth'>>=
validation_plot <- network_validation[
  , .(
    failure_rate = mean(failed),
    contagion_probability = mean(endogenous_withdrawals > 0)
  ),
...
ggplot(
  validation_plot,
  aes(reserve_ratio, failure_rate, color = model, linetype = model)
) +
  geom_line(linewidth = 0.75) +
  geom_point(size = 2) +
...
\begin{figure}[ht]
\centering
<<network-comparative-statics-figure, fig.width=7.8, fig.height=4.5, out.width='0.92\\textwidth'>>=
mono_plot_data <- merge(
  network_monotonicity,
  monotonicity_design[, .(structural_index, path, level)],
  by = "structural_index",
  all.x = TRUE
)[path != "baseline",
  .(failure_rate = mean(failed)),
  by = .(path, level, decision_model)
]
mono_plot_data[, path_label := factor(
  path,
  levels = c("reserve_ratio", "fixed_insurance", "shock_size"),
  labels = c("Reserve ratio", "Fixed insurance payment", "Shock size")
)]
mono_plot_data[, rule := factor(
  decision_model,
  levels = c("comparative", "threshold", "explicit_utility"),
  labels = c("Comparative recovery", "Bridge threshold", "Explicit utility")
)]
ggplot(
  mono_plot_data,
  aes(level, failure_rate, color = rule, linetype = rule)
) +
  geom_line(linewidth = 0.75) +
  geom_point(size = 1.9) +
  facet_wrap(~path_label, scales = "free_x", nrow = 1) +
  labs(
...
extreme partial-payment bounds agree in \Sexpr{nrow(bridge_agree)} of
...
agree on \Sexpr{fmt_pct(rules_validation$agreement)} of validation
failure outcomes and \Sexpr{fmt_pct(rules_monotonicity$agreement)} of
...
<<dd-expanded-table, results='asis'>>=
dd_appendix_table <- copy(dd_fixed_premium_summary)
dd_appendix_table[, withdrawal_premium :=
  sprintf("%.3f", withdrawal_premium)]
dd_appendix_table[, failure_rate := sprintf("%.3f", failure_rate)]
dd_appendix_table[, endogenous_withdrawals :=
  sprintf("%.2f", endogenous_withdrawals)]
setnames(
  dd_appendix_table,
  c(
    "withdrawal_premium",
...
\begin{figure}[ht]
\centering
<<dd-shift-figure, fig.width=7.5, fig.height=4.8, out.width='0.88\\textwidth'>>=
dd_shift_plot <- dd_shift_common[
  , .(mean_deposit = mean(optimal_deposit)),
  by = .(rho, withdrawal_premium, utility_shift)
]
ggplot(
  dd_shift_plot,
  aes(
  ...
  scale_color_brewer(palette = "Dark2", name = "Withdrawal premium") +
  labs(
    x = "Utility shift",
...
\Sexpr{fmt_pct(rules_confirmatory$agreement)} of matched
cell-replications. Their failure rates are
\Sexpr{fmt_pct(rules_confirmatory$threshold)} and
\Sexpr{fmt_pct(rules_confirmatory$explicit)}, a difference of
\Sexpr{fmt_pct(abs(rules_confirmatory$difference))}. These estimates
...
\begin{figure}[ht]
\centering
<<decision-rule-robustness-figure, fig.width=7.4, fig.height=4.5, out.width='0.86\\textwidth'>>=
robustness_plot <- data.table(
  design = factor(
    rep(c("Validation", "Prespecified", "Adaptive stress test"), each = 3),
    levels = c("Validation", "Prespecified", "Adaptive stress test")
  ),
  statistic = factor(
    rep(c("Outcome agreement", "Bridge-threshold failure",
          "Explicit-utility failure"), 3),
    levels = c("Outcome agreement", "Bridge-threshold failure",
               "Explicit-utility failure")
  ),
  value = c(
    rules_validation$agreement,
    rules_validation$threshold,
    rules_validation$explicit,
    rules_monotonicity$agreement,
    rules_monotonicity$threshold,
    rules_monotonicity$explicit,
    rules_confirmatory$agreement,
    rules_confirmatory$threshold,
    rules_confirmatory$explicit
  )
)
ggplot(robustness_plot, aes(design, value, fill = statistic)) +
  ...
  labs(x = NULL, y = "Share of matched runs") +
  theme_bw(base_size = 9.5) +
  theme(
