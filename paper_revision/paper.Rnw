...
insurance when commitment is imperfect. Our contribution to this
...
channel. The two information structures are empirically
...
global-games environments, but they are not usually the principal
policy margin in the standard formulation.
...
Our model is closest to this literature and extends the inference logic to a network setting:
